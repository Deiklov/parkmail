...
void init_segment(segment *segm, size_t _begin, size_t _len) {
    segm->begin = _begin;
    segm->len = _len;
    segm->m_ind = 0;
    segm->l_len = 0;
...
char *merge_segment(const char *arr, const segment *segm, const int segm_count) {
    size_t templen;
    size_t max_len = segm[0].m_len > segm[0].l_len ? segm[0].m_len : segm[0].l_len;///выбор между левым или центром
    size_t max_ind = max_len == segm[0].m_len ? segm[0].m_ind : 0;///либо 0 либо центр
    ...
    for (size_t i = max_ind; i < max_ind + max_len; ++i)
        buffer[i - max_ind] = arr[i];
    return buffer;
}

void child_proc(const char *arr, int deskript, int begin, size_t len) {
    segment segm;//deskript это дексриптор для записи в трубу
...
int *split_array(size_t cores, size_t arrsize) {
    if (arrsize < cores) {
    ...
    for (int i = 0; i < segm_count; ++i) {
        ...
        if (pid[i] == 0) {
            ...
            for (int j = 0; j < segm_count; ++j) {
                close(fd[2 * j]);
                close(fd[2 * j + 1]);//close copied deskriptors
            }
            exit(EXIT_SUCCESS);
        } else {
            if (pid[i] < 0) {
